...
- `CrossSellSection` - Client component (`'use client'`): self-contained cross-sell section. Fetches related products via `productService.getRelated(productId, limit)`, displays using `ProductGrid` with `grid-cols-2 md:grid-cols-4`. Props: `productId: string`, `limit?: number` (default 4), `className?`. Graceful degradation: renders `null` on error or empty. Cancellation token prevents stale state updates on unmount/prop change.
...
- `OrderSummary` - Presentational component: full order cost breakdown. Props: `subtotal`, `discountAmount`, `shippingCost`, `taxAmount`, `total`, `itemCount`, `appliedPromoCode: AppliedPromoCodeDetail | null`, `promoCodeMessage?: string`, `cartErrors: CartValidationError[]`, `isLoading`. Loading state: skeleton pulse with `role="status"` + `aria-busy="true"`. Conditional discount line (only when `discountAmount > 0`). Shipping shows "Free" when 0. Promo code Badge + discount description. Cart errors as destructive Alerts. Uses Card wrapper (self-contained for reuse). No internal state or service calls.
- `CheckoutPageContent` - Main checkout orchestrator at `/checkout`. `'use client'`, no props. Dual flow: guest (email + phone + inline address form) vs registered (`AddressSelector`). Hydrates both cartStore and authStore on mount. Redirects to `/cart` if empty. Calls `checkoutService.calculateTotals()` on mount and promo code changes. Passes backend-calculated totals to `OrderSummary`. "Place Order" button calls `orderService.create()` → redirects to Stripe `checkoutUrl`. Two-column responsive layout (`lg:grid-cols-3`). Guest address uses inline form fields (NOT AddressForm, which requires auth). Error handling via `ApiException` + Alert.
- `AddressSelector` - Address list + selection for registered users. Props: `onSelect: (address: Address) => void`. Fetches saved addresses via `addressService.list()`. Auto-selects default address on mount. Card-based UI with ring highlight for selected. "Add New Address" button reveals `AddressForm`. Error state with retry. Loading state.
- **Test fixtures**: `createOrderTotalResponse`, `createAppliedPromoCode`, `createCartValidationError`, `createAddress` in `components/checkout/testing/fixtures.ts`
...
- `/checkout` - Checkout page (Server Component wrapper + `CheckoutPageContent` client component): multi-step checkout flow. Guest: email + phone + inline address form. Registered: `AddressSelector` with saved addresses. Both: `PromoCodeInput` + `OrderSummary` with backend-calculated totals + "Place Order" → Stripe redirect. Redirects to `/cart` if empty.
...
- `productService` - Public product access. `list(params?)` → `ProductListResponse` (data + meta with pagination). `getBySlug(slug)` → `ProductDetailResponse` (data: ProductDetail with images + reviews). `getRelated(productId, limit?)` → `ProductListResponse` (calls `GET /products/{productId}/related`). Strips undefined params. Uses `NonNullable<operations['listProducts']['parameters']['query']>` for typed params.
...
- `checkoutService` - Order total calculation. `calculateTotals(items, promoCode?)` → `OrderTotalResponse`. Calls `POST /cart/calculate`. Unwraps envelope (`response.data.data`). Guards against empty items array (throws `ApiException` locally). Exports `OrderTotalResponse` and `OrderTotalRequest` types.
- `orderService` - Order creation. `create(data: CreateOrderRequest)` → `OrderCreateResult { orderId, orderNumber, checkoutUrl }`. Calls `POST /orders`. Unwraps envelope. Backend endpoint not yet implemented (Sprint 5+). Exports `OrderCreateResult` type.
...
- `CartPageContent` - Full-page cart view at `/cart`. `'use client'`, no props — reads from cartStore. Responsive `lg:grid-cols-3` layout: items column (col-span-2, `<ul>` of CartItem) + sticky sidebar (col-span-1) with `OrderSummary` (local cart data, no API call — zeros for discount/shipping/tax) + "Proceed to Checkout" link to `/checkout` + "Continue Shopping" link to `/products`. Empty state: ShoppingBag icon + "Your cart is empty" + Continue Shopping link. Hydrates store on mount. Includes `CrossSellSection` below cart items (full width, uses first item's productId).
